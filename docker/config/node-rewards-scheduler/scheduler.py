...
class ICCanisterClient:
    ...
    # Canister IDs
    NODE_REWARDS_CANISTER_ID = "uuew5-iiaaa-aaaaa-qbx4q-cai"  # Node rewards canister (DEV)
    ...
    async def get_rewards_daily(self, date_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            daily_results_type = Types.Record({
                ...
                'provider_results': Types.Vec(Types.Tuple(Types.Principal, daily_node_provider_rewards_type)),
            })
            ...
            if 'Ok' in result:
                ...
                logger.info(f"Successfully fetched rewards for {date_str} ({len(provider_results_dict)} providers)")
                return result_dict
            ...
            logger.warning(f"Failed to fetch rewards for {date_str}: {e}", exc_info=True)
            return {}
    
    async def get_latest_governance_reward_event(self) -> Optional[float]:
...
class NodeRewardsPusher:
    ...
    async def push_metrics_for_date(self, date_str: str) -> bool:
        """
        ...
        try:
            ...
            for provider_id, provider_rewards in provider_results.items():
                ...
                for node_result in provider_rewards.get('daily_nodes_rewards', []):
                    ...
                    failure_rate_data = self._unwrap_optional(node_result.get('daily_node_failure_rate'))
                    if isinstance(failure_rate_data, dict) and 'SubnetMember' in failure_rate_data:
                        # Extract node_metrics from SubnetMember variant
                        subnet_member = failure_rate_data['SubnetMember']
                        node_metrics = self._unwrap_optional(subnet_member.get('node_metrics'))
                        
                        if node_metrics:
                            subnet_id = self._unwrap_optional(node_metrics.get('subnet_assigned'))
            ...
            if metrics_lines:
                metrics_payload = '\n'.join(metrics_lines) + '\n'
                import_url = urljoin(self.victoria_url.rstrip('/') + '/', 'api/v1/import/prometheus')
                ...
                if response.status_code in (200, 204):
                    logger.info(f"✅ Successfully pushed data for {date_str} ({len(metrics_lines)} metrics)")
                    return True
                else:
                    logger.error(f"❌ Failed to push to VictoriaMetrics: {response.status_code} - {response.text}")
                    return False
            else:
    ...
    async def backfill(self, days: int = 40):
        ...
        for i in range(days, 0, -1):
            ...
            logger.info(f"[{days - i + 1:2d}/{days}] Backfilling data for {date_str}...")
            await self.push_metrics_for_date(date_str)
    ...
    async def run_daily_scheduler(self):
        ...
        while True:
            try:
                ...
                await self.push_metrics_for_date(yesterday)
...
async def main_async():
    ...
    # Set up environment
    pusher.setup_environment()
    
    # Wait for VictoriaMetrics
    pusher.wait_for_victoria_metrics()
    
    # Backfill historical data
    await pusher.backfill(days=40)
    
    # Run daily scheduler
    await pusher.run_daily_scheduler()


def main():
    """Main entry point"""
    logger.info("=" * 50)
    logger.info("Node Rewards Scheduler for VictoriaMetrics")
    logger.info("=" * 50)
    logger.info("")
